...
function createLLMConfigHandler(options = {}) {
    const { apiKeyEnvVar = 'ANTHROPIC_API_KEY', model = 'claude-3-5-sonnet-20241022', promptTemplate = DEFAULT_PROMPT_TEMPLATE, } = options;
    const anthropic = new sdk_1.default({
        apiKey: process.env[apiKeyEnvVar],
    });
    return async function llmConfigHandler(req, res, next) {
        try {
...
function extractRoutes(app) {
    ...
    function processRoute(route) {
        ...
        routes.push({
            ...
            params: route.keys?.map((k) => k.name),
        });
    }
